...
function FullscreenViewer({ url, onClose }: { url: string; onClose: () => void }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <ThreeScene url={url} controlsRef={controlsRef} defaultCameraState={defaultCameraState} interactive />
...
function MeshyV6Node({ id, data, selected }: NodeProps) {
  ...
  const textureConnections = useNodeConnections({ handleType: 'target', handleId: 'texture' })
  ...
  const handleRunModel = () => {
    if (d.onRunMeshyV6) {
      setLoading(true)
      setError(null)
      ;(d.onRunMeshyV6 as (nodeId: string) => Promise<string | null>)(id)
  ...
  return (
    <div className={`bg-[#1a1a1a] border border-[#27272A] rounded-xl shadow-xl ${locked ? 'ring-1 ring-neutral-700' : ''}`} style={{ width: NODE_WIDTH }}>
        ...
        {gridView && history.length > 0 ? (
          <GridView
            ...
            renderThumb={(_entry, i) => (
              <>
                <Box size={20} className={i === imageIndex ? 'text-orange-400' : 'text-neutral-500'} />
      ...
      <Handle type="target" position={Position.Left} id="input"
        ...
        title="Input" />
      <div className="absolute text-[10px] bg-black/30 backdrop-blur-sm px-1 rounded font-medium" style={{ top: 'calc(35% - 8px)', right: 'calc(100% + 18px)', color: '#00FFC5' }}>Input</div>

      <Handle type="target" position={Position.Left} id="texture"
        style={{ top: '65%' }}
        className={`!w-[7px] !h-[7px] !bg-[#00FFC5] !border-0 !-left-[9px] handle-green ${textureConnections.length > 0 ? 'connected' : ''}`}
        title="Texture" />
      <div className="absolute text-[10px] bg-black/30 backdrop-blur-sm px-1 rounded font-medium" style={{ top: 'calc(65% - 8px)', right: 'calc(100% + 18px)', color: '#00FFC5' }}>Texture</div>
...
export default memo(MeshyV6Node)
